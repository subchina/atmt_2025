...
def decode(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
    ...
    with torch.no_grad():
        # Compute the encoder output tensor E once
        enc_output = model.encoder(src_tokens, src_pad_mask)                  
    ...
    for t in range(max_out_len):
        ...
        output = model.decoder(generated, trg_pad_mask, enc_output, src_pad_mask).to(device)
...
def beam_search_decode(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
    ...
    beams = [(torch.tensor([[BOS]], dtype=torch.long, device=device), 0.0)]
    ...
    for _ in range(max_out_len):
        ...
        for seq, score in beams:
            ...
            with torch.no_grad():
                ...
                logits = model.decoder(seq, trg_pad_mask, enc_output, src_pad_mask)[:, -1, :]
